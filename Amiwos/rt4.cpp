...
AreaLight areaLight(
    Point(0, 24.3, 0),                // Centro del área
    Vector(10.0, 0, 0),               // Vector u (dimensión en X)
    Vector(0, 0, 10.0),               // Vector v (dimensión en Z)
    Color(50, 50, 50)                 // Radiancia
);
...
Color monteCarloArea(const Ray &r, int muestreos) {
	...
	if (id == 7) return spheres[id].emLuz;
    const Sphere &obj = spheres[id];
    ...
    for (int i = 0; i < muestreos; i++) {
        Point lightPoint = areaLight.randomPoint();
        Vector dir_to_light = (lightPoint - x).normalize();

        double cos_theta = n.dot(dir_to_light);
        if (cos_theta <= 0) continue; // Si está en dirección opuesta, ignorar
        ...
        if (!intersect(shadowRay, t2, id2) || id2 == 7) {
...
/*MODIFICACIONEEEEEEEEEEEEEEEEEEEEEEEES FIN*/ 
...
int main(int argc, char *argv[]) {
	...
	Color *pixelColors = new Color[w * h];
		...
		printf("------ Menú -----\n");
		printf("1.- Muestreo uniforme esférico\n");
		printf("2.- Muestreo uniforme hemisférico\n");
		printf("3.- Muestreo coseno hemisférico\n");
		printf("4.- Muestreo con fuente de área\n");
		printf("5.- Salir\n");
		...
		case 4: { // Nuevo caso: fuente de área
            #pragma omp parallel for
            for (int y = 0; y < h; y++) { 
                fprintf(stderr, "\r%5.2f%%", 100. * y / (h - 1));
                for (int x = 0; x < w; x++) {
