...
public:
	...
	double intersect(const Ray &ray) const {
    	Vector origen = ray.o - p; // origen del rayo hasta la posicion de la esfera


		// aplicacion del producto punto
    	double a = ray.d.dot(ray.d); 
    	double b = 2.0 * origen.dot(ray.d); 
    	double c = origen.dot(origen) - r*r;

    	double ecuacion= b*b-4*a*c; 

		// Determina si existe interseccion
    	if(ecuacion < 0){
        	return 0;
    	}
		else{
        	// Si hay, encuentra el parametro t donde ocurre la interseccion mas cercana
        	double t = (-b - sqrt(ecuacion)) / (2.0 * a);
