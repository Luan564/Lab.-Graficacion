...
// Definir el operador de impresión para Vector
...
public:
	...
	FuenteArea(Point c_, Vector e1_, Vector e2_, Color L_): centro(c_), eje1(e1_), eje2(e2_), emLuz(L_) {}

	Point PuntoAleatorio() const {
    ...
    double u = dis(gen);  // Número aleatorio en [0, 1]
    double v = dis(gen);  // Número aleatorio en [0, 1]
    
    return centro + eje1 * u + eje2 * v;  // Calcular el punto en la superficie
}
};
...
Color monteCarloHemisferico(const Ray &r, int muestreos, const FuenteArea &areaLight) {
    double t;
    int id = 0;
    Color radiancia;

    // Determinar intersección inicial
    if (!intersect(r, t, id))
        return Color();  // No hay intersección, devolver negro
    const Sphere &obj = spheres[id];

    // Si es una fuente de luz, devolver su radiancia
    if (id == 7) {
        return obj.emLuz;
    }
    ...
    Vector n = (x - obj.p).normalize();  // Normal a la superficie

    // // Depuración de la normal y el punto de intersección
    // std::cout << "Punto de intersección: " << x.x << ", " << x.y << ", " << x.z << std::endl;
    // std::cout << "Normal de la superficie: " << n.x << ", " << n.y << ", " << n.z << std::endl;

    // BRDF difusa (Lambertiana)
    Vector brdf = obj.c * (1.0 / M_PI);

    // Muestreo hemisférico para la fuente de área
    for (int z = 0; z < muestreos; z++) {
        // Generar un punto aleatorio en la fuente de área
        Point lightSample = areaLight.PuntoAleatorio();
        Vector lightDir = (lightSample - x).normalize();  // Dirección hacia el punto de luz
        double distanceSquared = (lightSample - x).dot(lightSample - x);

        Vector edge1 = areaLight.eje1;
        Vector edge2 = areaLight.eje2;
        // Calcular la normal de la fuente de área
        Vector lightNormal = (edge1 % edge2).normalize();

        // Depuración de la muestra de luz y la dirección de la luz
        // std::cout << "Muestra de luz (lightSample): " << lightSample.x << ", " << lightSample.y << ", " << lightSample.z << std::endl;
        // std::cout << "Dirección de la luz (lightDir): " << lightDir.x << ", " << lightDir.y << ", " << lightDir.z << std::endl;

        // Verificar visibilidad con un rayo secundario
        Ray shadowRay(x, lightDir);
        double t2;
        int id2 = 0;
        if (intersect(shadowRay, t2, id2) && t2 > 0 && t2 < distanceSquared) {
            continue;  // Si hay una intersección en la dirección de la luz, ignoramos esta muestra
        }

        // Calcular cosenos de los ángulos
        double cosThetaSurface = std::max(0.0, n.dot(lightDir));  // Punto hacia la luz
        double cosThetaLight = std::max(0.0, lightNormal.dot(lightDir));  // Luz hacia el punto

        // Depuración de los ángulos
        // std::cout << "cosThetaSurface: " << cosThetaSurface << ", cosThetaLight: " << cosThetaLight << std::endl;

        // Contribución de la muestra
        double area = (edge1 % edge2).length();  // Calcular el área de la fuente de luz

        // // Depuración de la contribución de luz
        // std::cout << "Contribución de luz: " 
        //           << (areaLight.emLuz * brdf) *
        //           (cosThetaSurface * cosThetaLight * (1.0 / (distanceSquared * muestreos))) *
        //           area << std::endl;

        if (cosThetaSurface > 0 && cosThetaLight > 0) {
            radiancia = radiancia + 
                        (areaLight.emLuz * brdf) *
                        (cosThetaSurface * cosThetaLight * (1.0 / (distanceSquared * muestreos))) *
                        area;
...
Color shadeH(const Ray &r, int muestreos, const FuenteArea &fuenteArea){
	return monteCarloHemisferico(r, muestreos, fuenteArea);
}
...
int main(int argc, char *argv[]) {
	...
	// Crear una fuente de área
	FuenteArea areaLight(
		Point(0, 50, 50),        // Centro de la fuente de luz
		Vector(20, 0, 0),        // Vector que define un lado del rectángulo
		Vector(0, 20, 0),        // Vector que define el otro lado del rectángulo
		Color(100, 100, 100)        // Intensidad de emisión
	);
		...
		case 2:{
			...
			for(int y = 0; y < h; y++) { 
				...
				for(int x = 0; x < w; x++ ) {
					...
					// computar el color del pixel para el punto que intersectó el rayo desde la camara
					pixelValue = shadeH(Ray(camera.o, cameraRayDir.normalize()), muestreos, areaLight);
